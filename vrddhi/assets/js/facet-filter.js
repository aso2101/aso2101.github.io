...
var facetButtonClass = "btn btn-sm btn-primary ml-1 mb-1";
function initFacets(){
  if(!hidableElems){
...
function updateFacets(){
	...
	// AO MODIFICATION: I want data-facet-XXXX to include MULTIPLE facets
        ...
        var valArray = x.map( function(){ 
			  var tags = this.getAttribute(dataAttr).split(' ');
			  return $.each(tags, function(index, value) {
			      '%%' +value;
			  });
		       });
        //sort values and remove order info
        var valArraySorted = $( valArray.sort() )
        ...
        var htmlArr = $(unique(valArraySorted)).map( function(){
                ...
                return '<div class="' + btnClass + '">' + this.replace('_',' ') + '</div>'; }
            );
...
function filterFacets(){
    ...
    facetPlaceholderElems.filter('[data-selected-facets]').each( function(){
        var type = $(this).data('facet-type'),
            dataSelector = 'data-facet-' + type,
            myRegexp = /%%([^%]+)%%/g,
            selectedStr = $(this).attr('data-selected-facets'),
