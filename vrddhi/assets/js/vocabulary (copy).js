...
function addHeader(metadata) {
    var row = $("<div class='row d-flex align-items-center'></div>"),
...
// try this custom sorting sometime
function customSort(x) {
    var ordering = {}, 
	sortOrder = ['a','ā','i','ī','u','ū','r̥','ē','ō','ai','au','k','kh','g','gh','ṅ','c','ch','j','jh','ñ','ṭ','ṭh','ḍ','ḍh','ṇ','t','th','d','dh','n','p','ph','b','bh','m','y','r','l','v','ś','ṣ','s','h'];
...
function makeTable(arrays) {
	...
	if (key == 0) { /* skip the first line */
	    ...
	    var toggle = $('<a href="javascript:void(0);" data-toggle="collapse" data-target="#div-'+key+'" aria-expanded="false"></a>').append("<span class='translit'>"+value[0]+"</span>"),
		dl = $('<dl class="row"></dl>'),
		dd = $('<dt class="col-3 d-flex align-items-center justify-content-end unhidden"></dt>').append(toggle),
		dt = $('<dd class="col-9 d-flex align-items-center justify-content-start unhidden"></dd>').append(value[1]),
		div = $('<div class="collapse notes" id="div-'+key+'"></div>');
	    if (value.length == 9) {
	    }
	    else if (value.length == 14) {
		var m = { };
		if (value[3].length > 0) { m["class"] = value[3]; }
		if (value[4].length > 0) { m["stem"] = value[4]; }
		if (value[5].length > 0) { m["gender"] = value[5]; }
		if (value[6].length > 0) { m["number"] = value[6]; }
		if (value[7].length > 0) { m["case"] = value[7]; }
		if (value[8].length > 0) { m["person"] = value[8]; }
		if (value[9].length > 0) { m["padam"] = value[9]; }
		if (value[10].length > 0) { m["root"] = value[10]; }
		if (value[11].length > 0) { m["preverb"] = value[11]; }
		if (value[12].length > 0) { m["l"] = value[12]; }
		if (value[13].length > 0) { m["gana"] = value[13]; }
		var word = { "morphology": m },
		    output = morphology(word),
		    lemma,
		    dictLinks = {
			"apte" : [ "https://dsal.uchicago.edu/cgi-bin/app/sanskrit_query.py?qs=","&searchhws=yes&matchtype=exact" ],
			"apte" : [ "http://dsalsrv02.uchicago.edu/cgi-bin/philologic/search3advanced?dbname=apte&query=", "&searchdomain=headwords&matchtype=exact&display=utf8" ],
		...
		switch(value[3]) {
		    case "verb":
		        lemma = value[11] + value[10];
		        break;
		    case "adj":
		        lemma = value[4].substr(0,value[4].indexOf(','));
		        break;
		    case "noun":
		        lemma = value[4];
		        break;
		...
		dt.append('<a href="'+dictLinks["mw"][0] + Sanscript.t(lemma,'iso','hk') + dictLinks["mw"][1]+'" target="_blank" class="ml-4 badge badge-pill badge-red"><small>m.-w.</small></a>');
		dt.append('<a href="'+dictLinks["apte"][0] + Sanscript.t(lemma,'iso','devanagari') + dictLinks["apte"][1]+'" target="_blank" class="ml-2 badge badge-pill badge-red"><small>apte/ddsa</small></a>');
		dt.append('<a href="'+dictLinks["sd"][0] + Sanscript.t(lemma,'iso','slp1') + dictLinks["sd"][1]+'" target="_blank" class="ml-2 badge badge-pill badge-red"><small>sd</small></a>');
		dt.append('<a href="'+dictLinks["v"][0] + Sanscript.t(lemma,'iso','hk') + dictLinks["v"][1]+'" target="_blank" class="ml-2 badge badge-pill badge-red"><small>vā.</small></a>');
		if (value[2].length > 0) {
		    div.append(value[2]); //notes
		}
		div.append(output);
	    } 
	    table.append(dl.append(dd).append(dt));
	    table.append(div);
	}
    });
